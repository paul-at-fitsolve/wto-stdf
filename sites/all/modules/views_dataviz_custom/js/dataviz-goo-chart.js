      ...
      if (settings.Dataviz) {
        ...
        for (var chartId in settings.Dataviz.chart) {
          ...
          if ($('#'+chartId, context).length) {
            ...
            data.addColumn('string', settings.Dataviz.chart[chartId].header[0]);
            for (var col in settings.Dataviz.chart[chartId].header) {
              if (col > 0){
            	data.addColumn('number', settings.Dataviz.chart[chartId].header[col]);
              }
            }
            data.addRows(settings.Dataviz.chart[chartId].rows);
           /*
            for (var i in settings.Dataviz.chart[chartId].rows) {
              var row = new Array();
              // Adding the rows.
